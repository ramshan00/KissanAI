...
```mermaid
flowchart LR
    subgraph Mobile["📱 Flutter Android APK"]
        UI[UI Screens] --> Service[ApiService]
        Service --> Provider[BookingProvider]
        Provider --> Audio[AudioRecorder]
        Provider --> Map[Flutter‑Map (OSM)]
        UI --> Permission[Permission Handler]
    end

    subgraph Backend["☁️ Hugging Face Space (FastAPI)"]
        REST[REST Endpoints]
        AI[Antigravity Gemini Orchestrator]
        DB[(SQLite DB)]
        REST --> AI
        AI --> DB
    end
    ...
    subgraph Ext["🔌 External Services"]
        Firebase[Firebase Auth – Phone OTP]
        Gemini[Google Gemini API]
        OSM[OpenStreetMap Tiles]
    end

    Service -->|HTTPS| REST
    UI -->|Firebase SDK| Firebase
    Map -->|Tile fetch| OSM
    AI -->|Gemini calls| Gemini
```
---
